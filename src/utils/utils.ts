...
export async function getNumberOfBugsPerMonth(year: any) {
  const session = await getServerSession(authOptions);
  const months = [
    'January', 'February', 'March', 'April', 'May', 'June',
    'July', 'August', 'September', 'October', 'November', 'December'
  ];
  const bugsPerMonth: any = {};
  for (const month of months) {
    const startDate = new Date(year, months.indexOf(month), 1);
    const endDate = new Date(year, months.indexOf(month) + 1, 0);
    ...
    bugsPerMonth[month] = bugCount;
  }
  return bugsPerMonth;
}

export async function getNumberOfBugsByStatus(status: any) {
  const session = await getServerSession(authOptions);
