...
export default function RegisterForm() {
  ...
  function validateRegisterForm() {
    ...
    if (formData.email.length < 5 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
      setErrorMessage(errorMessage => ({
        ...errorMessage,
        email: "Invalid email address. It must have at least 5 characters and be in a valid email format."
      }))
    }
    if (formData.password.length < 5 || !/[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]/.test(formData.password)) {
      setErrorMessage(errorMessage => ({
        ...errorMessage,
        password: "Password must have at least 10 characters and contain at least one special character"
      }))
    }
  ...
  async function signUpWithCredentials(event: any) {
    ...
    const data = await signIn("registerProvider", {
